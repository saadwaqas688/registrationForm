...
exports.registerUser = async (req, res) => {
  try {
    ...
    if (!req.body) {
      res
        .status(406)
        .json({ err: "You have to fill the registration form" });
      return;
    }
    ...
    if (!email || !password || !passwordCheck)
      return res
        .status(406)
        .json({ err: "Not all fields have been entered" });
    if (password.length < 8)
      return res.status(406).json({
        err: "The Password need to be at least 8 characters long",
      });
    if (password !== passwordCheck)
      res
        .status(406)
        .json({ err: "Password must be same for varification" });
...
exports.login = async (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(406)
        .json({ err: "No account with this email." });
    ...
    res
      .status(500)
      .json({ err: error.message || "Error while Login" });
  }
};
